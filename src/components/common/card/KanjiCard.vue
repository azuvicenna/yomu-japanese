...
<template>
    <div class="flip-card w-full h-75 relative cursor-pointer perspective-1000" :class="{ 'flipped': isFlipped }"
        ...
        <div class="flip-card-inner w-full h-full relative transition-transform duration-500 transform-style-3d">
            ...
            <div
                class="flip-card-front absolute w-full h-full bg-white border-4 border-slate-800 rounded-3xl flex flex-col items-center justify-center p-4 backface-hidden z-20 shadow-[8px_8px_0px_rgba(0,0,0,0.15)]">

                <span
                    class="absolute top-3 right-3 text-[10px] font-black px-2.5 py-1 rounded-full border-2 border-slate-800 bg-(--card-bg) text-slate-800 uppercase">
                    {{ data.level }}
                </span>
                ...
                <div class="text-center w-full" v-if="showFurigana">
                    <div class="mb-2">
                        ...
                        <p class="font-bold text-(--card-accent)">{{ data.onyomi }}</p>
                    </div>
            ...
            <div
                class="flip-card-back absolute w-full h-full bg-(--card-accent) border-4 border-(--card-dark) rounded-3xl flex flex-col items-center justify-center p-4 text-white backface-hidden z-10 shadow-[8px_8px_0px_rgba(0,0,0,0.15)] rotate-y-180">
